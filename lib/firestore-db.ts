...
/* ============================================================================
   DIRECT FIREBASE DATA BINDING WITH AUTOMATIC CLOUD FIRESTORE SEEDING
   ============================================================================ */
...
export async function getFirestoreServices(): Promise<ServiceCMS[]> {
  ...
  if (typeof window !== 'undefined') {
    Promise.all(INITIAL_SEED_DATA.services.map(s => saveFirestoreDoc('services', s.id, s))).catch(() => {});
  }
  return INITIAL_SEED_DATA.services as ServiceCMS[];
}

export async function getFirestoreProjects(): Promise<ProjectCRM[]> {
  let items = await fetchFirestoreCollection<ProjectCRM>('projects');
  if (items.length === 0) {
    if (typeof window !== 'undefined') {
      Promise.all(INITIAL_SEED_DATA.projects.map(p => saveFirestoreDoc('projects', p.id, p))).catch(() => {});
    }
    items = INITIAL_SEED_DATA.projects as ProjectCRM[];
  }
...
export async function getFirestoreTechStack(): Promise<TechStackCMS[]> {
  ...
  if (typeof window !== 'undefined') {
    Promise.all(INITIAL_SEED_DATA.techStack.map(t => saveFirestoreDoc('techStack', t.id, t))).catch(() => {});
  }
  return INITIAL_SEED_DATA.techStack as TechStackCMS[];
}
...
export async function getFirestoreTeam(): Promise<TeamMemberCMS[]> {
  ...
  if (typeof window !== 'undefined') {
    Promise.all(INITIAL_SEED_DATA.team.map(m => saveFirestoreDoc('team', m.id, m))).catch(() => {});
  }
  return INITIAL_SEED_DATA.team as TeamMemberCMS[];
}
...
export async function getFirestoreFAQs(): Promise<FAQItemCMS[]> {
  ...
  if (typeof window !== 'undefined') {
    Promise.all(INITIAL_SEED_DATA.faqs.map(f => saveFirestoreDoc('faqs', f.id, f))).catch(() => {});
  }
  return INITIAL_SEED_DATA.faqs as FAQItemCMS[];
}
...
export async function getFirestoreFeatures(): Promise<FeatureCMS[]> {
  ...
  if (typeof window !== 'undefined') {
    Promise.all(INITIAL_SEED_DATA.features.map(f => saveFirestoreDoc('features', f.id, f))).catch(() => {});
  }
  return INITIAL_SEED_DATA.features as FeatureCMS[];
}
...
export async function getFirestorePortfolio(): Promise<PortfolioItemCMS[]> {
  ...
  if (typeof window !== 'undefined') {
    Promise.all(INITIAL_SEED_DATA.portfolio.map(p => saveFirestoreDoc('portfolio', p.id, p))).catch(() => {});
  }
  return INITIAL_SEED_DATA.portfolio as PortfolioItemCMS[];
}
...
export async function getFirestoreTimeline(): Promise<TimelineCMS[]> {
  ...
  if (typeof window !== 'undefined') {
    Promise.all(INITIAL_SEED_DATA.timeline.map(t => saveFirestoreDoc('timeline', t.id, t))).catch(() => {});
  }
  return INITIAL_SEED_DATA.timeline as TimelineCMS[];
}
...
export async function getFirestoreOpenSource(): Promise<OpenSourceProjectCMS[]> {
  ...
  if (typeof window !== 'undefined') {
    Promise.all(INITIAL_SEED_DATA.openSourceProjects.map(os => saveFirestoreDoc('openSourceProjects', os.id, os))).catch(() => {});
  }
  return INITIAL_SEED_DATA.openSourceProjects as OpenSourceProjectCMS[];
}
...
export async function getFirestoreStats(): Promise<HeroStatCMS[]> {
  ...
  if (typeof window !== 'undefined') {
    Promise.all(INITIAL_SEED_DATA.heroStats.map((s, i) => saveFirestoreDoc('heroStats', 'stat-' + (i + 1), s))).catch(() => {});
  }
  return INITIAL_SEED_DATA.heroStats as HeroStatCMS[];
}
...
export async function getFirestoreValues(): Promise<CoreValueCMS[]> {
  ...
  if (typeof window !== 'undefined') {
    Promise.all(INITIAL_SEED_DATA.values.map((v, i) => saveFirestoreDoc('values', 'val-' + (i + 1), v))).catch(() => {});
  }
  return INITIAL_SEED_DATA.values as CoreValueCMS[];
}
...
export async function getFirestoreProcess(): Promise<ProcessStepCMS[]> {
  ...
  if (typeof window !== 'undefined') {
    Promise.all(INITIAL_SEED_DATA.processSteps.map((ps, i) => saveFirestoreDoc('processSteps', 'step-' + (i + 1), ps))).catch(() => {});
  }
  return INITIAL_SEED_DATA.processSteps as ProcessStepCMS[];
}
...
export async function getFirestoreSettings(): Promise<AgencySettingsCMS> {
  ...
  if (typeof window !== 'undefined') {
    saveFirestoreDoc('settings', 'agency', INITIAL_SEED_DATA.settings).catch(() => {});
    saveFirestoreDoc('settings', 'agency_settings', INITIAL_SEED_DATA.settings).catch(() => {});
  }
  return INITIAL_SEED_DATA.settings as AgencySettingsCMS;
}
